...
public class TektopiaAddons {
    ...
    public static HashSet<Block> oreBlocks; // Changed from BlockOre to Block
    ...
    public void postinit(FMLPostInitializationEvent postinit)
    {
        ...
        //Mining
        oreBlocks = new HashSet<Block>(); // Changed from BlockOre to Block
        ...
        for(Item item : items)
        {
            ...
            if(Arrays.stream(OreDictionary.getOreIDs(stack)).anyMatch(x -> OreDictionary.getOreName(x).startsWith("ore")))
            {
                ...
                if(block != null && block.getRegistryName() != null && block.getRegistryName().getPath().contains("fossil")) continue;
                if(block != null && block.getItemDropped(block.getDefaultState(), rand, 0) == item)
                {
                    oreItems.add(item);
        ...
        for(Block block : blocks)
        {
            ...
            // FIXED: Check for ANY block with "ore" in OreDictionary, not just BlockOre
            if(Arrays.stream(OreDictionary.getOreIDs(stack)).anyMatch(x -> OreDictionary.getOreName(x).startsWith("ore")))
            {
                oreBlocks.add(block); // Add any block that has "ore" in OreDictionary
                LOGGER.info("Found OreBlock: " + block.getRegistryName());
            }
        ...
        totalOreWeight = 0;
        for(Block ore : oreBlocks) // Changed from BlockOre to Block
        {
        ...
        {
            int[] ids = OreDictionary.getOreIDs(new ItemStack(ingot));
            if(ids.length > 0) {
                for(String s : ConfigHandler.MODDED_METAL_SMITH_PRIORITY)
                {
                    String[] split = s.split(":");
                    if(split.length == 2 && OreDictionary.getOreName(ids[0]).endsWith(split[0]))
                    {
                        smithIngotPriority.put(ingot, priorityStringMap.get(split[0]));
                    }
                }
            }
        }
    ...
    {
        // FIXED: Check if the block is in our oreBlocks collection (which now includes any ore)
